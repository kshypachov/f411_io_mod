...
- Update your MQTT broker URL, username, and password in the web interface.
...
- **`POST /api/firmware/upload`**
  ...
  - **Request**: Send `firmware.bin` with method "multiple posts" details https://mongoose.ws/documentation/tutorials/http/file-uploads/#binary-upload-multiple-posts" .
  - **Response**:
    ```json
    {
      "status": "success",
...
- **`POST /api/firmware/deactivate`**
  - **Description**: Deactivates the uplauded and activated firmware.
  - **Response**:
    ```json
    {
      "status": "success",
...
- **`POST /api/web_interface/remove`**
  - **Description**: Deletes all files from the web interface directory, it s needed for upload new version of web interface.
  - **Response**:
    ```json
    {
      "status": "success",
...
- **`POST /api/web_interface/upload`**
  ...
  - **Request**: Send files with method "multiple posts" details https://mongoose.ws/documentation/tutorials/http/file-uploads/#binary-upload-multiple-posts" ..
  - **Response**:
    ```json
    {
      "status": "success",
...
1. **MQTT Integration**: Once connected to a broker, the device will publish sensor data and subscribe to control topics. Configuration data is stored in `/settings/mqtt.conf` on SPI flash.
...
1. **MQTT Integration**: Once connected to a broker, the device will publish sensor data and subscribe to control topics. Configuration data is stored in `/settings/mqtt.conf`. You can edit this data via web interface.

2. **Tasks**:
   - **Ethernet Task**: Manages MQTT connectivity.
   - **IO Task**: Reads sensor states and controls output relays.
   - **Display Task**: For displaying data on an attached screen.
   - **Settings Task**: Manages persistent storage and reads configuration from flash memory.
